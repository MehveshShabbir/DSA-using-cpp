...
{	
    ...
    {	
	    ...
	    //-----------  if not -------------------
            ...
            {
	            //until stack is not empty AND top has high precedence --> add top element to postfix string	
            	while (!s.empty() && prcd(s.top(), infix[i]))
            	{
                 ...
                 //---  when stack is empty OR infix element is not ')' -->  add infix element to stack
            	 if(s.empty() || infix[i] != ')' )
                 {
                     s.push(infix[i]);
                 }
